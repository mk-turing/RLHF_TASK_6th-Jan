...
func EventHandler(dataStore *sync.Map, processedEvents sync.Map, updateChan chan Event) {
	for event := range updateChan {
		// Check if the event has already been processed
		if _, exists := processedEvents.Load(event.Key); exists {
		...
		processedEvents.Store(event.Key, true)
	}
}

func main() {
	// Create a synchronized map to store the data.
	dataStore := new(sync.Map)

	// Create a synchronized map to track processed events.
	processedEvents := new(sync.Map)
	...
	for i := 0; i < numHandlers; i++ {
		go EventHandler(dataStore, *processedEvents, updateChan)
	}
