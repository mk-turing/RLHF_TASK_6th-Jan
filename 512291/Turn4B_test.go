...
func BenchmarkSquareDirectWithProfile(b *testing.B) {
	f, err := os.Create("square_direct.prof")
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()
...
func BenchmarkSquareConcurrentWithProfile(b *testing.B) {
	runtime.GOMAXPROCS(runtime.NumCPU())
	results := make(chan float64, b.N)

	f, err := os.Create("square_concurrent.prof")
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()
