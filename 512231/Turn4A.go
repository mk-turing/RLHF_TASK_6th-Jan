...
func handleUser(user string) {
	...
	for {
		...
		if command == nil {
			...
			currentUser := user
			response := executeCommand(currentUser, command, args)
...
func main() {
	...
	// Create goroutines for each user to handle input concurrently.
	for _, u := range users {
		go handleUser(u.Name)
	}

	// Wait for all goroutines to finish.
	// In a real-world application, this would be replaced with a mechanism to handle user connections.
	select {}
}
