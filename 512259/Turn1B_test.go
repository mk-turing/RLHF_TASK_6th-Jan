...
// BenchmarkBubbleSort benchmarks the BubbleSort function with different array sizes and CPU configurations.
func BenchmarkBubbleSort(b *testing.B) {
	// Define different array sizes to benchmark
	sizes := []int{1000, 10000, 100000}
	// Define different CPU configurations
	cpus := []int{1, 2, runtime.NumCPU()}

	for _, cpusToUse := range cpus {
		// Set the number of CPUs for benchmarking
		runtime.GOMAXPROCS(cpusToUse)

		for _, size := range sizes {
			b.Run(fmt.Sprintf("cpus-%d/size-%d", cpusToUse, size), func(b *testing.B) {
