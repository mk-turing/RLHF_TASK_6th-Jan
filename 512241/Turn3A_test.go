...
func RunDynamicABTestWithStats(tests []ABTest, iterations int, analysisInterval time.Duration) (result map[string]float64, significance map[string]float64) {
...
// Benchmark function
func BenchmarkDynamicABTestWithStats(b *testing.B) {
	iterations := 1000
	...
	for i := 0; i < b.N; i++ {
		result, significance := RunDynamicABTestWithStats(tests, iterations, analysisInterval)
